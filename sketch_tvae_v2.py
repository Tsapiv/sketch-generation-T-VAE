...
class DecoderRNN(nn.Module):
    def __init__(self, fc_up):
        ...
        seqTransDecoderLayer = nn.TransformerDecoderLayer(d_model=self.latent_dim,
                                                          nhead=4,
                                                          dim_feedforward=hp.enc_hidden_size * 4,
                                                          dropout=hp.dropout,
                                                          activation="gelu")
    ...
    def forward(self, inputs, z, *args):
        ...
        timequeries = torch.ones(*inputs.shape[:-1], z.shape[-1], device=z.device)
...
class Model:
    def __init__(self):
        ...
        if use_cuda:
            self.encoder = EncoderRNN(self.fc_up).cuda()
            self.decoder = DecoderRNN(self.fc_up).cuda()
        else:
            self.encoder = EncoderRNN(self.fc_up)
            self.decoder = DecoderRNN(self.fc_up)
    ...
    def train(self, epoch):
        ...
        # decode:
        self.pi, self.mu_x, self.mu_y, self.sigma_x, self.sigma_y, self.rho_xy, self.q = self.decoder(batch_init, z)
        # prepare targets:
