...
class EncoderRNN(nn.Module):
    def __init__(self):
        ...
        # bidirectional lstm:
        self.transformer = nn.TransformerEncoderLayer(d_model=hp.enc_hidden_size, dim_feedforward=hp.enc_hidden_size * 4,
                                                      nhead=8)
        # self.transformer.linear2 = nn.Identity()
        # self.lstm = nn.LSTM(5, hp.enc_hidden_size, dropout=hp.dropout, bidirectional=True)
        # create mu and sigma from lstm's last output:
        self.fc_upscale1 = nn.Linear(5, hp.enc_hidden_size // 2)
        self.fc_upscale2 = nn.Linear(hp.enc_hidden_size // 2, hp.enc_hidden_size)
        self.relu = nn.ReLU()
    ...
    def forward(self, inputs, batch_size, hidden_cell=None):
        ...
        hidden = self.transformer(self.fc_upscale2(self.relu(self.fc_upscale1(inputs))))
        hidden = hidden[-1]
        # mu and sigma:
        mu = self.fc_mu(hidden)
        sigma_hat = self.fc_sigma(hidden)
...
class DecoderRNN(nn.Module):
    def __init__(self):
        ...
        self.fc_hc = nn.Linear(hp.enc_hidden_size, hp.Nz + 5)
        # unidirectional lstm:
        self.transformer = nn.TransformerDecoderLayer(d_model=hp.Nz + 5, dropout=hp.dropout, dim_feedforward=hp.dec_hidden_size * 4,
                                                      nhead=8)
        # self.transformer.linear2 = nn.Identity()
        # self.lstm = nn.LSTM(hp.Nz + 5, hp.dec_hidden_size, dropout=hp.dropout)
        # create proba distribution parameters from hiddens:
        self.fc_params = nn.Linear(hp.Nz + 5, 6 * hp.M + 3)
    ...
    def forward(self, inputs, z, hidden_cell=None):
        ...
        outputs = self.transformer(inputs, torch.unsqueeze(self.fc_hc(hidden_cell), 0))
        # in training we feed the lstm with the whole input in one shot
        # and use all outputs contained in 'outputs', while in generate
        # mode we just feed with the last generated sample:
        if self.training:
            y = self.fc_params(outputs.view(-1, hp.Nz + 5))
        else:
            y = self.fc_params(outputs[-1].view(-1, hp.Nz + 5))
...
class Model:
    ...
    def train(self, epoch):
        ...
        # encode:
        z, self.mu, self.sigma, h = self.encoder(batch, hp.batch_size)
        ...
        # decode:
        self.pi, self.mu_x, self.mu_y, self.sigma_x, self.sigma_y, self.rho_xy, self.q = self.decoder(inputs, z, h)
        # prepare targets:
    ...
    def conditional_generation(self, epoch):
        ...
        for i in range(Nmax):
            ...
            # decode:
            self.pi, self.mu_x, self.mu_y, self.sigma_x, self.sigma_y, self.rho_xy, self.q = self.decoder(
                input, z, h)
            # hidden_cell = (hidden, cell)
