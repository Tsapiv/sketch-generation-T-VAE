...
class EncoderRNN(nn.Module):
    ...
    def forward(self, x, Seq_Len=None):
        x = pack_padded_sequence(x, Seq_Len.to('cpu'), enforce_sorted=False)
...
class EncoderTrans(nn.Module):
    def __init__(self, hp: HParams, dmodel=512):
...
class DecoderTrans(nn.Module):
    def __init__(self, hp: HParams, dmodel=512):
        ...
        self.fc_hc = nn.Linear(hp.z_size, dmodel)
        self.emb = nn.Linear(5, dmodel)
        self.pos_encoder = PositionalEncoding(dmodel)
        decoder_layers = nn.TransformerDecoderLayer(dmodel, 4, hp.dim_feedforward)
        decoder_norm = torch.nn.LayerNorm(dmodel)
        self.decoder = torch.nn.TransformerDecoder(decoder_layers, 2, decoder_norm)
    ...
    def forward(self, inputs, z_vector, seq_len=None, hidden_cell=None, isTrain=True):
        ...
        #
        # if seq_len is None:
        #     seq_len = torch.tensor([1]).type(torch.int64).to('cpu')
        # else:
        #     seq_len = seq_len.to('cpu')

        # outputs = self.decoder(self.pos_encoder(x), self.fc_hc(z_vector))
        hidden = None
        if self.training:
            x = self.emb(inputs[:, :, :5].permute(1, 0, 2)).permute(1, 0, 2)
            outputs = self.decoder(self.pos_encoder(x), self.fc_hc(z_vector)[None])
            ...
            hidden = torch.cat([hidden_cell, inputs], dim=0) if hidden_cell is not None else inputs
            x = self.emb(hidden[:, :, :5].permute(1, 0, 2)).permute(1, 0, 2)
            outputs = self.decoder(self.pos_encoder(x), self.fc_hc(z_vector)[None])
            y_output = self.fc_params(outputs[-1][None].permute(1, 0, 2))
        ...
        return [z_pi.reshape(-1, 20), z_mu1.reshape(-1, 20), z_mu2.reshape(-1, 20),
                z_sigma1.reshape(-1, 20), z_sigma2.reshape(-1, 20), z_corr.reshape(-1, 20),
                z_pen_logits.reshape(-1, 3)], hidden
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, inputs, z_vector, seq_len=None, hidden_cell=None, isTrain=True):
        ...
        return [z_pi.reshape(-1, 20), z_mu1.reshape(-1, 20), z_mu2.reshape(-1, 20),
                z_sigma1.reshape(-1, 20), z_sigma2.reshape(-1, 20), z_corr.reshape(-1, 20),
                z_pen_logits.reshape(-1, 3)], (hidden, cell)
