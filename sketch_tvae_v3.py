...
class DecoderRNN(nn.Module):
    def __init__(self, embed_dim=hp.enc_hidden_size, latent_dim=hp.enc_hidden_size, dropout=hp.dropout, nhead=1, num_layers=1,
        ...
        self.tgt_mask = nn.Transformer.generate_square_subsequent_mask(Nmax + 1)
    ...
    def forward(self, inputs, z, *args):
        ...
        if self.training:
            self.tgt_mask = self.tgt_mask.to(z.device)
            outputs = self.decoder(tgt=timequeries, memory=z, tgt_mask=self.tgt_mask)
        else:
...
class Model:
    ...
    def kullback_leibler_loss(self, mu, sigma):
        ...
        return hp.wKL * self.eta_step * LKL #torch.max(LKL, KL_min)
